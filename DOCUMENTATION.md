...
当前存量：`packages/desktop/spec/`（auto-update-removal、client-config-rollout-removal、resource-telemetry-removal、renderer-crash-recovery）、`packages/shared/spec/`（conversation-delta-batch-apply、conversation-turn-directory）、`packages/server/spec/local-exposure-hardening.md`、`packages/zcode-cua/spec/computer-use-restore.md`（Computer Use 恢复：线协议契约、本地运行时边界与验收）。
