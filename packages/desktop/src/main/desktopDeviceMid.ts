 ...
 *
 * 该文件历史上与数仓上报（telemetryCore）/ ARMS 共用同一个 `deviceMid` 字段；远端遥测
 * 已下线，但 billing 与 CLI / 远端 server 仍读取同一持久化 UUID，文件名与字段保持不变。
 * 窗口创建前需同步取值（经 preload `--device-id=` 注入），故此处用 node:fs 同步读写。
 *
 * 竞态规避：
